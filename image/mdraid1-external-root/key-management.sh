...
manage_encryption_key() {
    local key_method="$IGconf_image_key_method"
    local key_size="$IGconf_image_key_size"
    local boot_mount="$1"
    ...
    case "$key_method" in
        random)
            ...
            ;;
        file)
            if [[ -z "${IGconf_image_key_file:-}" ]]; then
                echo "ERROR: Key file not specified for method=file"
                exit 1
            fi
            encryption_key=$(read_key_from_file "${IGconf_image_key_file}")
            ;;
        env)
            if [[ -z "${IGconf_image_key_env:-}" ]]; then
                echo "ERROR: Environment variable not specified for method=env"
                exit 1
            fi
            encryption_key=$(read_key_from_env "${IGconf_image_key_env}")
            ;;
        tpm)
