# Шаблон Bash скрипта для rpi-image-gen

## Структура файла: [path]/[script-name].sh

```bash
#!/bin/bash
# ==========================================
# RPI-IMAGE-GEN: [SCRIPT_NAME]
# ==========================================
#
...
- Название и описание скрипта
- Возможности и примеры использования
- Информация об авторе и версии

### 2. Настройки
- Режимы выполнения (set -euo pipefail)
- Константы и цвета

### 3. Глобальные переменные
- Флаги для режимов выполнения
...
#!/bin/bash
# ==========================================
# RPI-IMAGE-GEN LIBRARY: [LIBRARY_NAME]
# ==========================================

# Функции должны экспортироваться
function_name() {
    # Реализация функции
}
...
log_info "Выполнение этапа: ${stage}"
...
if ! some_command; then
    log_error "Команда завершилась с ошибкой"
    exit 1
fi
...
```bash
# Проверка зависимостей
validate_dependencies "command1" "command2"

# Проверка файлов
validate_file "/path/to/file" "конфигурационный файл"

# Проверка каталогов
validate_dir "/path/to/dir" "рабочий каталог"
```
...
if [[ -n "${VARIABLE_NAME:-}" ]]; then
    log_debug "Переменная установлена: ${VARIABLE_NAME}"
fi

# Сохранение переменных
echo "VARIABLE_NAME=${VARIABLE_NAME}" >> config.env
```

## Shellcheck

Для проверки скриптов рекомендуется использовать shellcheck:
```bash
shellcheck script.sh
```

Комментарии для shellcheck:
```bash
# shellcheck disable=SC2016  # Игнорировать проверку выражений
# shellcheck source=lib/common.sh  # Указать источник файла
```
